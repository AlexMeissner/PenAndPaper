...
namespace Server.Services.BusinessLogic
{
    ...
    public class Monster(IRepository<DbMonster> monsterRepository) : IMonster
    {
        public async Task<int> Create(MonsterDto payload)
        {
            var monster = new DbMonster()
            {
                Name = payload.Name,
                Size = payload.Size,
                Type = payload.Type,
        ...
        {
            var monster = await monsterRepository.FirstAsync(x => x.Id == id);
        ...
        {
            var monsters = monsterRepository.Select(x => new MonsterDto(
                x.Id,
                x.Name,
                x.Size,
                x.Type,
            ...
            var a = await monsters.ToListAsync();
